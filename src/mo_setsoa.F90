...
!-----------------------------------------------------------------------
  ...
  if ( rxn_soa(6) <= 0 ) then
     rxn_soa(6) = get_rxt_ndx( 'BIGALK_OH' )
  end if
  if( all( rxn_soa(:) < 1 ) ) then
     ...
     if (masterproc) then
        write(iulog,*) '-----------------------------------------'
        write(iulog,*) 'soa_inti_old: mozart will do soa aerosols'
        write(iulog,*) '-----------------------------------------'
     endif
  end if
...
!
real(r8) function soa_yield(m_0,xalpha,xk)
!
  implicit none
!
  real(r8), intent(in) :: m_0
...
!
  ...
  subroutine soa_inti_equil(pbuf2d)
    ...
    rxn_soa(6) = get_rxt_ndx( 'TOLO2_NO' )
    if (rxn_soa(6)<0) then
    ...
    if( all( rxn_soa(:) < 1 ) ) then
       ...
       if (masterproc) then
          write(iulog,*) '-----------------------------------------'
          write(iulog,*) 'soa_inti_equil: mozart will do soa aerosols'
          write(iulog,*) '-----------------------------------------'
       endif
    end if
  ...
  subroutine setsoa_equil(dt,reaction_rates,tfld,vmr,xhnm,ncol,lchnk,pbuf)
    ...
    call pbuf_get_field(pbuf, fracsoa_ndx, fracsoa )
    call pbuf_get_field(pbuf, fracsog_ndx, fracsog )
